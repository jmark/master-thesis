...
We conduct turb sim in periodic box.
The isothermal driven turbulence simulation consists of two phases. At the
beginning of the run turbulent forcing, explained in \sec{turbuforcing}, stirs
the medium up to a certain root-mean-square velocity which is equivalent to the
sonic Mach number $\mach_{\text{forcing}}$ (cf. \sec{governing-equation}/Choice
...
distinct sawtooth profile appears.}
...
\fig{plots/stirturb/snapshots/forcing-stop-01.png}. In contrast to the others
...
Apparently, the large scale modes of the driving force field (cf. \sec{turbuforcing})
...
\image{1.0}{plots/stirturb/snapshots/forcing-stop-01.png}{Column Sonic Mach Number and
column density along z-axis at the time when forcing is stopped: $t_d = 4.0$.}

\image{1.0}{plots/stirturb/snapshots/forcing-stop-02.png}{Column Sonic Mach Number and
column density along z-axis at the time when forcing is stopped: $t_d = 4.0$.}

\image{1.0}{plots/stirturb/snapshots/forcing-stop-03.png}{Column Sonic Mach Number and
column density along z-axis at the time when forcing is stopped: $t_d = 4.0$.}

\image{1.0}{plots/stirturb/snapshots/decay-01.png}{Column Sonic Mach Number and
column density along z-axis during decay phase: $t_d = 5.0$.}

\image{1.0}{plots/stirturb/snapshots/decay-02.png}{Column Sonic Mach Number and
column density along z-axis during decay phase: $t_d = 5.0$.}

\image{1.0}{plots/stirturb/snapshots/decay-03.png}{Column Sonic Mach Number and
column density along z-axis during decay phase: $t_d = 5.0$.}
...
(\fig{plots/stirturb/evolutions/ekin-dissipation.png}) which would just yield
...
\fig{plots/stirturb/evolutions/ekin-dissipation.png}.  The rate of internel
...
the internal energy deprivation by meassuring the energy loss right before
...
log-normal fits of \eqn{density-pdf} under \sec{theory-density-pdf}. The notorious
...
6.0$. As the decay progresses the width of
the distributions decrease and the mean values return to the intial density:
...
in \eqn{density-pdf-mach} breaks down. \Tbl{pdf-dens-vw} lists the fitting results
...
density PDFs of the turbulence. The dashed line represents the root-mean-square
velocity of the PPM solver and serves as a reference. Due to the lognormal
relationship the fit yielded very low error margins. Hence, the error bars are
to small to see.}
\begin{table}[H]
...
runaway. Compare \fig{plots/stirturb/pdfs/evol-sonic-mach.png}.}
\centering
\begin{tabular}{lcccc}
\toprule
...
\sec{sec:computational-frameworks}) is working correctly. More distinctive is
...
side in \fig{plots/stirturb/pdfs/vels-vw-dyntimes.png} is gone due to a small
mass distribution in the high-velocity range.}
...
which were introduced in \sec{theory-powerspectra}. The powerspectrum is
...
quantitiy (cf. \sec{theory-powerspectra}). In
\fig{plots/stirturb/powerspectra/ekin-dyntimes.png} it then be the total
squared kinetic energy. \Tbl{stirturb-pws} proofs via numerical integration
...
Consulting the right-most column in \tbl{stirturb-pws} the relative amount of
...
Besides the lognormality of the density PDF (\sec{stirturb-pdf}) the
...
\Tbl{stirturb-pws} lists the results of the linear fits of the intertial range
...
energy budget in \fig{plots/stirturb/evolutions/ekin-dissipation-cumsum.png}
...
\fig{plots/stirturb/pdfs/vels-mw-turb-phase.png}. Estimating the energy in the
dissipative range, see \tbl{stirturb-pws}, also supports this observation.
