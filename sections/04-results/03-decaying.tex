...
FLASH to FLEXI (cf. \sec{trafo}), the initial state was slightly smoothed beforehand via
...
the amount of FV elements rises again. From $t_d = 1$ onward the ratio
...
vinite volume elements to the total number of elements. \remark The other
...
The course of the total energy in \fig{plots/decayturb/evol-ener} is plotted in
...
the dissipated kinetic energy (\fig{plots/decayturb/evol-ekin-diss}). See
discussion under \sec{stirturb-energy-dissipation}. Right after the simulation has
...
not show anything surprising besides the fact that there is no significant bulk
motion (cf. \sec{stirturb-pdf}).
...
Gaussian blur truncated the high density range. The log-normal fit (dotted
...
the lognormal relationship, the fit yields very low error margins. Hence, the
...
yield sensible results. However, the Mach number estimation via relation
\eqn{density-pdf-mach} ($b = 2/3$) is systematically below the reference.
This agrees with the results in the decay phase of the driven turbulence (cf.
\sec{stirturb-pdf}).}
...
dispersion of the PDFs over time indicates no bulk motion which is a good sign
(cf. \sec{stirturb-pdf}).}
...
In agreement with \cite{kitsionas2009algorithmic}
...
There is no clear winner, who resolves small scale structures best.  On the one
hand, FLASH's solvers contain more energy in the dissipative range of the
kinetic energy powerspectra (cf. \fig{plots/decayturb/pws-ekin-dyntimes})
...
\fig{plots/decayturb/pdf-vw-dens.png}). However, one should not forget that all
...
\tbl{stirturb-pws}). It would also explain why FLEXI shows less features in the
